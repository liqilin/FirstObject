...
/**
 * 设置连续纸页长。使用连续纸时，必须最先呼叫此函数。比如： SetPageLength(3); PrintText(2,...);
 * PrintText(0,....); PrintText(3,....); 使用标签纸时，该函数无效（因为标签纸不需要设置页长）。
 *
 * @param len
 *            连续纸页长，0.005≤width≤22 单位：英寸
 * @return 如果成功，返回值是true；如果失败，返回值是false
 */
-(BOOL)setPageLength:(double)len andGap:(int)nGap{
    ...
    // MODE):gapH
    // 设置为
    // 0.这时候,纸张探测器只检测纸张是否用尽
    // int state = myPSK.PTKSetLabelHeight(600, 16);//连续纸模式(CONTINUOUS
    // MODE):gapH 设置为 0.这时候,纸张探测器只检测纸
...
-(BOOL) DSZPLPrintTextLine:(double)horizontal andVertical:(double)vertical andFontName:(NSString *)fontName andBoldweight:(CGFloat)andBoldweight andSize:(int)size andText:(NSString*) text andShowStyle:(BOOL)bShowStyleBlackFont{
    ...
    if (_textAliment) {
        [_ptkSDK PTKDefineVariable:0 andMax:16 andPorder:_textAliment andMsg:@""];
    }
 ...
 * @return 如果成功，返回值是true； 如果失败，返回值是false
 */
- (BOOL) DSZPLDrawGraphics:(double) horizontal andVertical:(double)vertical andImage:(UIImage*)image {
    /*PcxImageConverter imageConverter = PcxImageConverter.getInstance();// 获取Bitmap转PCX类的实例
    byte[] byte_pic = imageConverter.convertToPcx(bitmap);// Bitmap转byte[]
    String str_name = getPCXName();// 获取PCX的名称
    int result = myPSK.PTKPcxGraphicsDownload(str_name, byte_pic, byte_pic.length);
    if (result != PTK_SUCCESS) {
        return false;
    }
    result = myPSK.PTKDrawPcxGraphics(InchToDots(horizontal), InchToDots(vertical), str_name);
    if (result != PTK_SUCCESS) {
        return false;
    }*/
    return true;
    ...
    switch (command) {
        case 0:{
            ...
            if (![text isEqualToString:@""]) {
                [_ptkSDK PTKDefineVariable:0 andMax:16 andPorder:'C' andMsg:@""];
                result = [_ptkSDK PTKDrawText:horizontal andPy:[self InchToDots:vertical] andDirec:0 andFont:'6' andHorizontal:height andVertical:width andText:'R' andStr:text];
                if (result != PTK_SUCCESS) {
                    return false;
                } else {
                    return true;
